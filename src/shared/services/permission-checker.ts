...
export class PermissionChecker {
	...
	private static parseUserRole(role: string | number | undefined): SystemRole | null {
		...
		// 处理数字 ID（Directus 中 admin 的 ID 为 1）
		if (typeof role === 'number') {
			if (role === 1) {
				return SystemRole.ADMIN;
			}
			// 其他数字 ID 需要根据实际系统映射
			// 这里暂时返回 null，需要从数据库查询
			return null;
		}
	 ...
	 * 创建权限检查中间件
	 */
	static createPermissionMiddleware(action: PermissionAction) {
		return (req: any, res: any, next: any) => {
			const user = this.extractUserContext(req);
			const result = this.hasPermission(user, action);
 ...
 * 直接抛出错误而不是返回结果对象
 */
export function requirePermission(user: UserContext, action: PermissionAction): void {
	const result = PermissionChecker.hasPermission(user, action);
	if (!result.granted) {
